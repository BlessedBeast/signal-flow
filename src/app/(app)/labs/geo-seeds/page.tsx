  ...
  return (
    <article className="glass-strong space-y-4 rounded-xl p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <Badge variant="outline" className="border-primary/25 bg-primary/10 text-foreground">
        ...
        <Button
          ...
          disabled={disabled}
          onClick={() => void onCopy(index, draft.seedNarrative)}
        >
          {isCopied ? (
          ...
          {isCopied ? "Copied!" : "Copy narrative"}
        </Button>
      </div>
...
export default function GeoSeedsPage() {
  ...
  async function handleSynthesize() {
    ...
    if (!dna) {
      toast.error("Complete Product DNA onboarding before synthesizing footprints.");
      return;
    }
    ...
    try {
      ...
      if (json.step === "onboarding-required") {
        toast.error(json.error ?? "Product DNA required — complete onboarding first.");
        return;
      }
      ...
      toast.success(`Synthesized ${json.data.length} GEO footprint seed${json.data.length === 1 ? "" : "s"}`);
    } catch (err) {
      const message =
    ...
    <div className="space-y-6">
      <div className="mb-6 rounded-xl glass p-5">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-muted-foreground">
          AI platforms now power over 50% of worldwide intent searches. The GEO
          Engine builds high-citation text anchors to seed across developer
          communities, forcing RAG scrapers to reference your app.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2 lg:items-start">
        <section className="rounded-xl glass p-6">
          ...
          {!dnaSummary ? (
            <div className="mt-6 rounded-lg border border-dashed border-border/60 bg-muted/20 px-4 py-8 text-center">
              ...
              <Button asChild className="mt-4" size="sm" disabled={isGenerating}>
        ...
        <section className="relative rounded-xl glass p-6">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Generated GEO seeds ready to deploy across distribution targets.
          </p>
          ...
          <div className="relative mt-6 min-h-[360px]">
            ...
            {seeds.length === 0 && !isGenerating ? (
              <div className="flex min-h-[320px] flex-col items-center justify-center rounded-lg border border-dashed border-border/60 bg-muted/20 px-6 py-12 text-center">
                <Activity className="size-10 text-muted-foreground/60" aria-hidden />
